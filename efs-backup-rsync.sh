...
for myContent in `sudo ls -a --ignore . --ignore .. /mnt/backup/ | awk 'NR%'$numClients==$clientNum`; do
  echo "sudo rsync -s -ah --stats --delete --numeric-ids --log-file=/tmp/efs-backup.log /mnt/backup/$myContent /mnt/backups/$efsid/$interval.0/"
